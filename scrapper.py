...
games = pd.read_csv('data/cal-season-17.csv')

ab = pd.read_csv('data/abreviations.csv', index_col=0)

for _, g in games.iterrows():
    time = datetime.strptime(g['Date'], '%a %b %d %Y').strftime('%Y%m%d')
    visitor = ab.loc[g['Visitor/Neutral'], 'Abbreviation']
    home = ab.loc[g['Home/Neutral'], 'Abbreviation']
    match_id = time + '-' + visitor + '-' + home
